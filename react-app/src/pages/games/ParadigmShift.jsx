...
const PHASES = [
  { id: "pre", label: "Pre-Science", color: "#6a6a7a", desc: "No dominant paradigm; competing schools" },
  { id: "normal", label: "Normal Science", color: "#4a7aaa", desc: "Puzzle-solving within accepted framework" },
  { id: "anomaly", label: "Anomaly", color: "#aa8a30", desc: "Puzzle that resists standard solutions" },
  { id: "crisis", label: "Crisis", color: "#aa5a30", desc: "Paradigm openly questioned; rules loosen" },
  { id: "revolution", label: "Revolution", color: "#7aaa4a", desc: "New paradigm displaces the old" },
];
    ...
    return (
      <div style={P.root}>
        <div style={P.doneWrap}>
          <div style={P.doneCircle}>{score}<span style={P.doneMax}>/{maxScore}</span></div>
          <div style={P.doneTitle}>Scientific Revolutions Mapped</div>
          <div style={P.doneSub}>
            {pct >= 80 ? "Kuhn would call you a revolutionary scientist."
             : pct >= 55 ? "Solid — you understand the structure of normal science."
  ...
  return (
    <div style={P.root}>
      <div style={P.topBar}>
        ...
        <span style={P.progress}>{idx + 1} / {cases.length}</span>
      </div>
      ...
      <div style={P.question}>Which Kuhnian phase does this describe?</div>

      <div style={P.phaseGrid}>
        {PHASES.map((ph) => {
          let style = P.phaseBtn;
          if (selected === ph.id && !submitted) style = { ...style, ...P.phaseBtnSelected, borderColor: ph.color, color: ph.color };
          if (submitted && ph.id === c.correct) style = { ...style, ...P.phaseBtnCorrect, borderColor: ph.color, color: ph.color, background: ph.color + "18" };
          if (submitted && ph.id === selected && ph.id !== c.correct) style = { ...style, ...P.phaseBtnWrong };
          if (submitted && ph.id !== c.correct && ph.id !== selected) style = { ...style, opacity: 0.3 };
          return (
            <button key={ph.id} style={style} onClick={() => handleSelect(ph.id)} disabled={submitted}>
              <div style={{ ...P.phaseDot, background: submitted && ph.id === c.correct ? ph.color : selected === ph.id ? ph.color : "#333" }} />
              <div>
      ...
      {!submitted && (
        <button
          style={{ ...P.submitBtn, opacity: selected ? 1 : 0.4, cursor: selected ? "pointer" : "default" }}
      ...
      {submitted && !isCorrect && (
        <div style={P.wrongBox}>
          <div style={P.wrongTitle}>Not quite. The correct phase is highlighted above.</div>
          <button style={P.retryBtn} onClick={handleRetry}>Try again</button>
        </div>
      )}

      {submitted && isCorrect && (
        <div style={P.correctBox}>
          <div style={P.correctTitle}>✓ Correct — {PHASES.find(p => p.id === c.correct)?.label}</div>
