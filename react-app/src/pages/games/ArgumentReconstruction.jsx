...
export default function ArgumentReconstruction() {
    ...
    return (
      <div style={T.root}>
        <div style={T.doneCard}>
          ...
          <div style={T.doneScore}>Score: {score} / {ARGUMENTS.length * 3}</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={T.root}>
      ...
      <div style={T.task}>
        Drag the premises into the correct logical order. The conclusion is fixed.
      </div>
      ...
      {submitted && !won && (
        <div style={T.feedback}>
          <div style={T.fbTitle}>Not quite — review the highlighted premises and try again.</div>
          <button style={T.retryBtn} onClick={handleReset}>Reorder & Retry</button>
        </div>
      )}
      ...
      {submitted && won && (
        <div style={T.wonBox}>
          ...
          {!showNote && (
            <button style={T.hintBtn} onClick={() => setShowNote(true)}>
          ...
          <button style={T.nextBtn} onClick={handleNext}>
            {argIdx + 1 < ARGUMENTS.length ? "Next Argument →" : "Finish"}
          </button>
        </div>
      )}
    </div>
