...
export default function GameButterflyJob() {
  ...
  function choose(idx) {
    ...
    setLog((arr) => [...arr, { yr: scenario.yr, choice: choice.t, dv: choice.dv }])
  ...
  const barColor = pickedChoice?.dv === 0 ? '#3a6840' : pickedChoice?.dv >= 80 ? '#8a2525' : '#6b5018'
        ...
        <div className="butterfly-game">
          {screen === 'intro' && (
            <div className="intro">
              ...
              <div className="intro-body">
                <p>You have been dropped into seven moments across the twentieth century. Each time, you are a minor figure — a laundress, a newspaper seller, a guard on a corridor. You are not the subject of the history.</p>
                <p>You will make a small decision. The kind that barely registers at the time. You will not know, until after you&apos;ve made it, <strong>what was riding on it.</strong></p>
                <p>Seven decisions. Some will leave the timeline as you found it. Some won&apos;t. At the end, a report.</p>
              </div>
              <button className="start-btn" onClick={startGame}>Begin — 1907</button>
            </div>
          )}
            ...
            <>
              <div className="progress">
                {SCENARIOS.map((_, i) => (
                  <div key={i} style={{ display: 'contents' }}>
                    ...
                    <div className={`pdot ${i < cur ? 'done' : i === cur ? 'now' : ''}`} />
                  </div>
                ))}
                <div className="pline" />
                <span className="pcnt">{cur + 1} / {SCENARIOS.length}</span>
              </div>
              ...
              {phase === 'decide' && (
                <>
                  ...
                  <div className="role-text">{scenario.role}</div>
                  <div className="ctx-block">{scenario.ctx}</div>
                  <p className="question">{scenario.q}</p>
                  <div className="choices">
                    {scenario.ch.map((c, i) => (
                      <button className="ch" onClick={() => choose(i)} key={i}>
                        <div className="ch-t">{c.t}</div>
                        <div className="ch-s">{c.s}</div>
                      </button>
                    ))}
                  </div>
                </>
              ...
              {phase === 'consequence' && pickedChoice && (
                <>
                  <div className="choices" style={{ marginBottom: '1.25rem' }}>
                    {scenario.ch.map((c, i) => (
                      <button key={i} className={`ch ${picked === i ? 'chosen' : ''}`} disabled>
                        <div className="ch-t">{c.t}</div>
                        <div className="ch-s">{c.s}</div>
                      </button>
                    ))}
                  ...
                  <div className="con-box">
                    <div className="con-top">
                      <div className="con-title">{pickedChoice.ct}</div>
                      <span className={`tag tag-${pickedChoice.tc}`}>{pickedChoice.tg}</span>
                    </div>
                    <div className="chain">
                      {pickedChoice.chain.map((step, i) => (
                    ...
                    <div className="dbar-wrap">
                      <div className="dbar-label">
                        <span>Divergence from recorded history</span>
                        <span>{pickedChoice.dv}%</span>
                      </div>
                      <div className="dbar-track">
                        <div className="dbar-fill" style={{ background: barColor, width: `${barWidth}%` }} />
                      </div>
                    </div>
                  </div>
                  <button className="next-btn" onClick={advance}>
                    {cur < SCENARIOS.length - 1 ? 'Continue through time →' : 'See the final report →'}
                  </button>
                </>
              )}
            </>
          ...
          {screen === 'end' && (
            <div className="end-page">
              ...
              <p className="end-sub">Total divergence: {total} points across {SCENARIOS.length} decisions</p>
                  ...
                  {log.map((entry, i) => {
                    ...
                    return (
                      <div className="log-row" key={i}>
                        <span className="log-yr">{entry.yr}</span>
                        <span className="log-ch">{entry.choice}</span>
                        <span className={`log-dv ${dvClass}`}>{dvLabel}</span>
                      </div>
                    )
                  })}
                </div>
              </div>
              <button className="replay-btn" onClick={replay}>Travel again ↺</button>
            </div>
          )}
        </div>
      </div>
